...
- Use install_dense_ann only when the user wants 384-d ANN installed/backfilled (default algorithm=hnsw, quantization=dense; rebuild=true drops and recreates; product needs product_num_subvectors). A successful non-empty backfill stamps configured_model so retrieve_text can resolve the Viewer MiniLM provider.
